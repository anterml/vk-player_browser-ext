function startVKPlay() {
  ...
  function observePlayer() {
    ...
    observerPlayer.observe(document.getElementById("mv_layer_wrap"), {
      childList: true,
      subtree: true,
    })
  }
  ...
  function start() {
    ...
    if (typeof clearControl === "function") clearControl()

    const playerBox = document.getElementById("video_player")
    if (!playerBox) return console.log("not playerbox", playerBox)

    const video = playerBox.querySelector("video")
    if (!video) return console.log("no video elem")
    ...
    window.addEventListener("mouseup", () => {
      if (isMousedown) {
        ...
        if (saveSpeed === 1) textSpeedElem.style.display = "none"
...
function startVKvideoPlay() {
  ...
  window.addEventListener(
    "mousedown",
    () => {
      const newPlayerBox = document.getElementById("video_player")
      if (!newPlayerBox)
        return console.log("playerBox element not found", newPlayerBox)

      const newVideo = newPlayerBox.querySelector("video")
      if (!newVideo) return console.log("video element not found")

      if (playerBox !== newPlayerBox && video !== newVideo) {
  ...
  function start() {
    ...
    const stopPlayKeys = ")0"

    // Смена скорости при нажатии на цифры
    const keypress = e => {
      const pos =
        ruSpeedKeys.indexOf(e.key) + 1 || engSpeedKeys.indexOf(e.key) + 1
      if (pos || (e.key >= 1 && e.key <= 9)) {
        const speed = 0.25 * (pos || e.key)
        video.playbackRate = speed
      ...
      } else if (stopPlayKeys.includes(e.key)) {
        video[video.paused ? "play" : "pause"]()
      }
    }
...
function startYoutubePlay() {
  function tryGetTimecodes() {
    ...
    if (!data) return console.log("no youtube data", youtubeData)

    const [head, tail] = data.split("00:00")
    if (!tail) return console.log("no tail")
    ...
    const times = result.map(value => {
      ...
      return (
        Number(seconds) + Number(minutes) * 60 + Number(hours || 0) * 60 * 60
      )
    })
  ...
  if (!video)
    return console.info(
      "Browser plugin 'vk/youtube/rutube player speed': the youtube player is not found",
    )
  ...
  const keypress = e => {
    ...
    if (e.shiftKey && pos) {
      ...
      if (nextTimeCode) video.currentTime = nextTimeCode
    }
  }
...
function startTwitchPlay() {
  ...
  if (!playerBox)
    return console.info(
      "Browser plugin 'vk/youtube/rutube player speed': playerBox is not found",
    )
...
if (window.location.host.includes("vk.com")) {
  ...
  startVKPlay()
} else if (window.location.host.includes("vkvideo.ru")) {
  console.log("start vkvideos player")
  startVKvideoPlay()
